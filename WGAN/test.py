...
os.makedirs('images_CIFAR10_noBN/test/real', exist_ok=True)
os.makedirs('images_CIFAR10_noBN/test/fake', exist_ok=True)
...
parse.add_argument('--model_dir', type=str, default='images_CIFAR10_noBN/model/199_generated_image.pkl', help='save models here')
...
def main():
    ...
    dataloader = DataLoader(datasets.CIFAR10('../../data/CIFAR10', train=False,
    ...
    for i, (image, labels) in enumerate(dataloader):
        ...
        z = Variable(FloatTensor(np.random.normal(loc=0.0, scale=1.0, size=(opt.batch_size, opt.latent_dim, 1, 1))))
        ...
        save_image(gen_image.data, 'images_CIFAR10_noBN/test/fake/%d_generated_image.png' % (i), nrow=1, normalize=True)
        save_image(real_images.data, 'images_CIFAR10_noBN/test/real/%d_real_image.png' % i, nrow=1, normalize=True)

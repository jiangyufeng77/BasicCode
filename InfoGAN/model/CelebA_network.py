...
class G_CelebA(nn.Module):
    def __init__(self, n_classes, channels):
        ...
        self.model = nn.Sequential(
            nn.ConvTranspose2d(228, 448, kernel_size=2, stride=1, bias=False), # 1->2
            nn.BatchNorm2d(448),
            nn.ReLU(),
    ...
    def forward(self, noise, label):
        input = torch.cat((noise, self.label_emb(label)), -1)
        input = input.view(input.shape[0], 228, 1, 1)
...
class Q_net(nn.Module):
    ...
    def forward(self, img):
        ...
        classes = classes.view(img.shape[0], 100)
        # latent_code = self.latent_code(output_Q)
        # latent_code = latent_code.view(img.shape[0], )

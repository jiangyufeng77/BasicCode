...
os.makedirs('images_MNIST/test/real', exist_ok=True)
os.makedirs('images_MNIST/test/fake', exist_ok=True)
...
parse.add_argument('--model_dir', type=str, default='images_MNIST/model/199_generated_image.pkl', help='save models here')
...
def main():
    ...
    for i, (image, labels) in enumerate(dataloader):
        ...
        save_image(gen_image.data, 'images_MNIST/test/fake/%d_generated_image.png' % (i), nrow=1, normalize=True)
        save_image(real_images.data, 'images_MNIST/test/real/%d_real_image.png' % i, nrow=1, normalize=True)
